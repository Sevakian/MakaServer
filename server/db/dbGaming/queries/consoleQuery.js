...
dbConnection.addConsole = (value) => {
  ...
  return new Promise((resolve, reject) => {
    dbConnection.query(
      `INSERT INTO konsole (konsolename, bild, hersteller, erscheinungsjahr, generation, status, typ, anzahlverkauft, text) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        name,
        bild,
        hersteller,
...
dbConnection.getConsoles = () => {
  return new Promise((resolve, reject) => {
    dbConnection.query(
      `SELECT * FROM konsole ORDER BY Typ DESC, Erscheinungsjahr DESC`,
      (err, results) => {
        if (err) {
          return reject(err);
        }
